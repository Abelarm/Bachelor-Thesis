...
public class Automata implements OrientedGraph<Integer,String> {
	...
	public class MyVertex<Integer> extends MyPosition<Integer> implements Vertex<Integer> {
		...
		public Edge<String> insertIngoingEdges(Edge<String> e) {
			IngoingEdges.add(e);				//aggiunge questo arco alla fine della nostra lista
	...
	//CLASSSE MYEDGString INTStringRNA
	...
	public class MyEdge<String> extends MyPosition<String> implements Edge<String> {
		protected MyVertex<Integer>[] endIntegerertices; 				//array che contiente i vertici estremi dell'arco
		...
		MyEdge (Vertex<Integer> v, Vertex<Integer> w, String o) {
			...
			endIntegerertices = (MyVertex<Integer>[]) new MyVertex[2];			//istanza l'array degli archi su cui è incidente
			endIntegerertices[0] = (MyVertex<Integer>)v;						//come primo elemento dell'arco mette il primo vertice passato in input, quello da cui esce
			endIntegerertices[1] = (MyVertex<Integer>)w;						//come secondo elemento dell'arco mette il secondo vertice passato in input
	...
	private MyEdge<String> checkEdge(Edge<String> e) {
		try{
			if(e.equals(null)){
				System.out.println("la lancio qui?");
					throw new InvalidPositionException();
			}
	...
	public boolean haveOutgoingEdge(Vertex<Integer> v,String s){
		MyVertex<Integer> ver=this.checkVertex(v);
	...
	/**public boolean haveIngoingEdge(Vertex<Integer> v,String s){
		MyVertex<Integer> ver=this.checkVertex(v);
	...
	public Edge<String> getOutgoingEdge (Vertex<Integer> v,String s) throws NullEdgeException{
		if(haveOutgoingEdge(v, s)){
			MyVertex<Integer> ver=this.checkVertex(v);
			Edge<String> a=ver.OutgoingEdges.get(s);
			return a;
		}else
