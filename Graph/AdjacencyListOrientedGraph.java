...
public class AdjacencyListOrientedGraph<V, E> implements OrientedGraph<V, E> {
	...
	public class MyVertex<V> extends MyPosition<V> implements Vertex<V> {
		protected PositionList<Edge<E>> OutgoingEdges;
		protected PositionList<Edge<E>> IngoingEdges;
		protected Position<Vertex<V>> loc;

		public MyVertex(V o){
			element = o;
			OutgoingEdges = new NodePositionList<Edge<E>>();
			IngoingEdges = new NodePositionList<Edge<E>>();
		}
		...
		public Iterable<Edge<E>> OutgoingEdges() {
			return OutgoingEdges;
		}
		
		/**Restituisce la collezzione iterabile degli archi incidenti sul nodo su cui è richiamato**/
		public Iterable<Edge<E>> IngoingEdges() {
			return IngoingEdges;
		}
		
		/**Inserisce un nuovo arco entrante al nodo su cui è richiamato**/
		public Position<Edge<E>> insertIngoingEdges(Edge<E> e) {
			IngoingEdges.addLast(e);				//aggiunge questo arco alla fine della nostra lista
			return IngoingEdges.last();				//restituisce l'ultimo che corrisponde proprio alla position di edge Position<Edge<E>>
		}
		
		/**Inserisce un nuovo arco uscente al nodo su cui è richiamato**/
		public Position<Edge<E>> insertOutgoingEdges(Edge<E> e) {
			OutgoingEdges.addLast(e);				//aggiunge questo arco alla fine della nostra lista
			return OutgoingEdges.last();				//restituisce l'ultimo che corrisponde proprio alla position di edge Position<Edge<E>>
		}
		
		/**Rimuove un arco dalle lista di incidenze **/
		public void removeIncidence(Position<Edge<E>> p) { 
			try{
				OutgoingEdges.remove(p);						//prova a rimuoverlo dalla lista degli archi uscenti
			}catch(InvalidPositionException e){					//se viene lanciata l'eccezione(l'arco nonn fa parte della lista degli archi uscenti)
				IngoingEdges.remove(p);							//lo rimuove dalla lista degli archi entranti
			}
		}
	...
	public class MyEdge<E> extends MyPosition<E> implements Edge<E> {
		...
		MyEdge (Vertex<V> v, Vertex<V> w, E o) {
			...
			endVertices[0] = (MyVertex<V>)v;						//come primo elemento dell'arco mette il primo vertice passato in input, quello da cui esce
		...
		public void setIncidences(Position<Edge<E>> dacuiesce, Position<Edge<E>> dacuientra) {
			Inc[0] = dacuiesce;
			Inc[1] = dacuientra;
		}
	...
	public Vertex<V> nextNode(Vertex<V> v, Edge<E> e) throws InvalidPositionException {
		...
		//inserire controllo if se è uguale a endVertices[0]
	...
	public boolean areAdjacent(Vertex<V> u, Vertex<V> v) throws InvalidPositionException {
		...
		Iterable<Edge<E>> tosearch=ver1.OutgoingEdges();				//cerco gli archi uscenti dal primo noodo
		for(Edge<E> x: tosearch){										//per ogni arco
	...
	public Edge<E> insertEdge(Vertex<V> dacuiesce, Vertex<V> dacuientra, E o) throws InvalidPositionException {
		MyEdge<E> ee=new MyEdge<E>(dacuiesce, dacuientra, o);									//crea il nuovo arco con i vertici e l'elemento passato in input
		...
		MyVertex<V> out=this.checkVertex(dacuiesce);										//controlla e casta a MyVertex il primo vertice passato in input
		MyVertex<V> inc=this.checkVertex(dacuientra);										//controlla e casta a MyVertex il secondo vertice passato in input
	...
	public V removeVertex(Vertex<V> v) throws InvalidPositionException {
		MyVertex<V> temp=this.checkVertex(v);				//controlla e casta a MyVertex il nodo passato in input
		...
		Iterable<Edge<E>> list=temp.OutgoingEdges();		//lista iterabile degli archi uscenti da quel vertice da eliminare
		for(Edge<E> x: list)								//per ogni arco appartenente a list (e quindi uscente sul vertice da eliminare)
	...
	public boolean Cammino(Vertex<V> v, int k){
		...
		Iterable<Edge<E>> listaedge=this.OutgoingEdges(temp);
		for(Edge<E> e: listaedge){
			MyVertex<V> ver=this.checkVertex(this.nextNode(temp, e));
			ver.put("Visitato", "Si");
	  ...
	  /**prende in input un vertice v e restituisce una collezione iterabile dei vertici del grafo
	    * che sono adiacenti a v. */
	    ...
	    Vertex<V> opp = null;
	    for (Edge<E> e : vertex.OutgoingEdges()) { // scorro tutti gli archi uscenti a v
